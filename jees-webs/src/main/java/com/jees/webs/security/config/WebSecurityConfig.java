...
//    IVerifyService     verifyService;
    ...
    private void _configure_tpl_notaccess_( HttpSecurity _hs, String _url, Template _tpl, List< SuperMenu > _menus ) throws Exception{
        _hs.authorizeRequests().antMatchers( _url ).permitAll();
        //
...
//        _hs.csrf().ignoringAntMatchers( csrf_url + "/**" );
    ...
    private void _configure_tpl_( HttpSecurity _hs ) throws Exception{
        List< String > tpl_names   = iTS.getTemplateNames();
        Template       tpl_default = iTS.getDefaultTemplate();

        for( String tpl : tpl_names ){
            List< SuperMenu > menus      = iTS.loadTemplateMenus( tpl );
            ...
            // 没有栏目配置页面添加默认权限
            if( !tpl_access ){
                _configure_tpl_notaccess_( _hs, tpl_url, iTS.getTemplate( tpl ), menus );
            }else {
                ...
                for (SuperMenu m : menus) {
                    ...
                    if (m.isPermit() && !tpl_access) {
                        ...
                        List<String> list = null;//absSS.loadMenuRoles(m.getId());
    ...
    protected void configure( HttpSecurity _hs ) throws Exception{
        ...
        if( securityService.isEnable() ) {
            ...
            _hs.sessionManagement()
                    .maximumSessions( CommonConfig.getInteger( "jees.webs.security.maxSession", 1000 ) )
...
//        sDB.initialize();
        ...
        if ( CommonConfig.getBoolean( "jees.webs.verify.enable", false ) ) {
            ...
            if ( CommonConfig.get( "jees.webs.verify.model", "local" ).equalsIgnoreCase( "local" ) )
                _configure_tpl_( _hs );
            else
                _configure_base_(_hs);
        } else {
            if ( CommonConfig.getBoolean( "jees.webs.install.enable", false ) ) {
